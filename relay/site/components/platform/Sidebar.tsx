...
function SidebarNav({ online, collapsed }: { online?: boolean; collapsed: boolean }) {
  ...
  return (
    <>
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-2 obs-scroll">
      ...
      <div className="border-t border-border p-3">
        {!collapsed ? <p className="mb-2 px-1 text-[10px] uppercase tracking-wider text-muted-2">Relay</p> : null}
        <div className={clsx("flex items-center gap-2 rounded-xl border border-border bg-bg-1 px-3 py-2.5", collapsed && "justify-center px-2")}>
          {!collapsed ? (
            <StatusPill kind={relayKind} size="sm" />
          ) : (
            <span className={clsx("h-2 w-2 rounded-full", online !== false ? "bg-green" : "bg-red")} />
          )}
        </div>
      </div>
    </>
...
export function Sidebar({ online }: { online?: boolean }) {
  ...
  return (
    <>
      <aside
        className={clsx(
          "panel-raised relative z-30 hidden h-full shrink-0 flex-col border-r border-border md:flex",
      ...
      <aside
        className={clsx(
          "panel-raised fixed inset-y-0 left-0 z-50 flex w-[min(280px,88vw)] flex-col border-r border-border transition-transform duration-200 md:hidden",
