...
local Library = {
	...
	Scheme = {
		...
		Font = Font.new("rbxasset://fonts/families/RobotoMono.json", Enum.FontWeight.Medium),
...
function Library:Notify(...)
	...
	if not Notification.Persist then
		local Time = tonumber(Notification.Time) or 5
	...
	function Window:AddDialog(Idx, Info)
		...
		function Dialog:Resize()
			...
			for _, BtnWrap in FooterButtonsList do
				HasButtons = true
				ButtonCount += 1
				TotalButtonWidth += BtnWrap.Container.Size.X.Offset
			end
...
function Library:SetTheme(Theme)
	if type(Theme) == "string" and Library.Themes then
