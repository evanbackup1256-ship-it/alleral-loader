(() => {
  ...
  gate.innerHTML = `
    <div class="cf-gate-card">
      ...
      <p class="cf-gate-lead">Verifying your connection before loading Alleral Hub.</p>
      <ul class="cf-steps" id="cfSteps">
        <li data-step="browser"><span class="cf-check"></span> Checking browser</li>
        <li data-step="human"><span class="cf-check"></span> Verifying request</li>
  ...
  if (siteKey) {
    ...
    script.async = true;
    script.onload = () => {
      gate.querySelector("#cfTurnstile")?.classList.remove("hidden");
      window.turnstile.render(gate.querySelector("#cfTurnstile"), {
        sitekey: siteKey,
        theme: "dark",
        callback: () => { clearInterval(stepTimer); complete(); },
        "error-callback": complete,
      });
    };
    script.onerror = () => setTimeout(complete, 2000);
